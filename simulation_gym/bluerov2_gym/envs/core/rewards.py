...
class Reward:
    ...
    def get_reward(self, obs):
        ...
        orientation_error = abs(np.pi/2 -
            np.arctan2(obs["target_y"][0]- obs["y"][0],obs["target_x"][0] - obs["x"][0] )  
        - obs["theta"][0])
        ...
        reward = (
            ...
            -2 * position_error_z # weight for z error
            - velocity_weight * velocity_penalty  # Weight for velocity
            - 25*orientation_error  # Weight for orientation  ### pi= 3.14
            + buffer_zone_penalty
        )
        print(f"Reward components: "
              f"- Position error (disk): {(-1.0 * position_error_disk):.2f}, "
              f"- Position error (z): {(-1.5 * position_error_z):.2f}, "
              f"- Velocity penalty: {(-velocity_weight * velocity_penalty):.2f}, "
              f"- Orientation error: {(-20 * orientation_error):.2f}, "
              f"- Buffer zone penalty: {buffer_zone_penalty:.2f}"
              f"Position: {obs['x'][0]:.2f}, {obs['y'][0]:.2f}, {obs['z'][0]:.2f}, ")
        
        print(f"x: {obs['x'][0]:.2f}, y: {obs['y'][0]:.2f}, theta: {obs['theta'][0]:.2f}, orientation: {orient}")
