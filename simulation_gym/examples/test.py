...
def test_agent():
    ...
    model = PPO.load("examples/12_mio/bluerov_ppo_scratch2")

    # Create a dummy vec env for proper normalization
    vec_env = DummyVecEnv([lambda: gym.make("BlueRov-v0")])
    vec_env = VecNormalize.load("examples/12_mio/bluerov_vec_normalize_scratch2.pkl", vec_env) #forgot to save it, skip for now
